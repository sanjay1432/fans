...
var  getTokenFromDB = function (userId, userType,token, callback) {
    ...
    async.series([
        function (cb) {
            ...
            else if (userType == Config.APP_CONSTANTS.DATABASE.USER_ROLES.FANSPICK){
                Service.FanspickService.getUser(criteria,{},{lean:true}, function (err, dataAry) {
                    if (err){
                        cb(err)
                    }else {
                        if (dataAry && dataAry.length > 0){
    ...
    async.series([
        function (cb) {
             if (userType == Config.APP_CONSTANTS.DATABASE.USER_ROLES.USER){
                 Service.FanspickService.updateUser(criteria,setQuery,{new:true}, function (err, dataAry) {
                     if (err){
                         cb(err)
                     }else {
...
var verifyFanspickToken = function (token, callback) {
    var response = {
        valid: false
    };
    Jwt.verify(token, Config.APP_CONSTANTS.SERVER.JWT_SECRET_KEY, function (err, decoded) {
        if (err) {
            callback(Config.APP_CONSTANTS.STATUS_MSG.ERROR.ACTIVE_SESSIONS_EXPIRED);
        } else {
            getFanspickTokenFromDB(decoded.id, decoded.type,token, callback);
        }
    });
};

var getFanspickTokenFromDB = function (userId, userType,token, callback) {
    var userData = null;
    var criteria = {
        _id: userId,
        accessToken : token
    ...
    async.series([
        function (cb) {
            if (userType == Config.APP_CONSTANTS.DATABASE.USER_ROLES.FANSPICK){
                Service.FanspickService.getUser(criteria,{},{lean:true}, function (err, dataAry) {
                    if (err){
                        cb(err)
                    }else {
...
module.exports = {
    ...
    verifyFanspickToken: verifyFanspickToken,
